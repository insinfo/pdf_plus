...
class PkiServer {
  ...
  final List<AsymmetricKeyPair<PublicKey, PrivateKey>>
      tsaCertChain; // Leaf first
  ...
  Future<void> _handleOcsp(HttpRequest request) async {
    ...
    responseBytes.add(ASN1ObjectIdentifier.fromComponentString(
    ...
    // signatureAlgorithm
    basicResponse.add(PkiBuilder.createAlgorithmIdentifier(
        '1.2.840.113549.1.1.11')); // sha256WithRSA

    // signature
    // We sign tbsResp.encodedBytes
    final sig = PkiBuilder.signData(
        tbsResp.encodedBytes, tsaKeyPair.privateKey as RSAPrivateKey);
  ...
  Future<void> _handleTimestamp(HttpRequest request) async {
    ...
    encap.add(ASN1ObjectIdentifier.fromComponentString(
    ...
    messageImprint.add(PkiBuilder.createAlgorithmIdentifier(
