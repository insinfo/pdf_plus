...
XPathSequence _fnInnermost(XPathContext context, XPathSequence nodes) {
  final nodeList =
      nodes.map((item) => item.toXPathNode()).whereType<XmlNode>().toList();
...
XPathSequence _nodeSetOperation(
  ...
  final arg1 =
      arg1Seq.map((item) => item.toXPathNode()).whereType<XmlNode>().toSet();
  final arg2 =
      arg2Seq.map((item) => item.toXPathNode()).whereType<XmlNode>().toSet();
