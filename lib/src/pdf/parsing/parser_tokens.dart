...
class PdfParserTokens {
  static const List<int> endStreamToken = <int>[
    0x65, 0x6E, 0x64, 0x73, 0x74, 0x72, 0x65, 0x61, 0x6D // endstream
  ];

  static String decodePdfString(Uint8List bytes) {
  ...
  static ({String value, int nextIndex}) readName(
    Uint8List bytes,
    int i,
    int end,
  ) {
  ...
  static bool matchToken(Uint8List bytes, int index, List<int> token) {
  ...
  static ({Uint8List? id, int nextIndex}) readIdArray(
    Uint8List bytes,
    int start,
    int end,
  ) {
  ...
  static ({Uint8List bytes, int nextIndex}) readHexString(
    Uint8List bytes,
    int start,
    int end,
  ) {
    int i = start;
    if (bytes[i] != 0x3C) {
      throw StateError('Invalid hex string.');
    }
    i++;
    final hex = <int>[];
    ...
    if (i >= end) throw StateError('Invalid hex string.');
    i++;
    return (bytes: hexToBytes(hex), nextIndex: i);
  }

  static Uint8List hexToBytes(List<int> hexBytes) {
  ...
  static ({Uint8List bytes, int nextIndex}) readLiteralString(
    Uint8List bytes,
    int start,
    int end,
  ) {
  ...
  static int lastIndexOfSequence(
    Uint8List bytes,
    List<int> pattern,
    int start,
    int end,
  ...
  static int skipPdfWsAndComments(Uint8List bytes, int i, int end) {
    if (i < end) {
      final b = bytes[i];
  ...
  static ({int value, int nextIndex}) readIntFast(
    Uint8List bytes,
    int start,
    int end,
  ) {
  ...
  static bool matchBytes(Uint8List bytes, int offset, List<int> target) {
  ...
  static int skipTokenRaw(Uint8List bytes, int i, int end) {
    i = skipPdfWsAndComments(bytes, i, end);
    if (i >= end) return i;
  ...
  static ({int value, int nextIndex}) readInt(Uint8List bytes, int i, int end) {
    if (i >= end) {
      throw StateError('Unexpected EOF while reading integer.');
    }
    var neg = false;
    if (bytes[i] == 0x2B /* + */) {
      i++;
  ...
  static bool isDigit(int b) => b >= 0x30 && b <= 0x39;

  static bool isHexDigit(int b) =>
      (b >= 0x30 && b <= 0x39) ||
      (b >= 0x41 && b <= 0x46) ||
      (b >= 0x61 && b <= 0x66);
  ...
  static bool isDelimiter(Uint8List bytes, int index) {
