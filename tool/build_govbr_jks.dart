...
const String _defaultOutputFile =
    r'test/assets/truststore/gov.br/cadeia_govbr_unica.jks';
const String _defaultPassword = '12345678';
...
void main(List<String> args) {
  ...
  for (final entity in dir.listSync(recursive: true)) {
    ...
    for (final der in ders) {
      ...
      if (!seenDerBlobs.add(derKey)) continue;
  ...
  final jks = JksKeyStore.load(_minimalJks(), storePassword: '');
  jks.entries
    ..clear()
    ..addAll(entries);
  final jksBytes = jks.save(password);

  final out = File(outputFile);
  out.parent.createSync(recursive: true);
  out.writeAsBytesSync(jksBytes, flush: true);

  final reloaded = JksKeyStore.load(jksBytes, storePassword: password);

  stdout.writeln('JKS gerado com sucesso.');
...
String _buildAlias(X509Certificate cert) {
  ...
  final shortSerial =
      serial.length > 12 ? serial.substring(serial.length - 12) : serial;
...
void _printHelp() {
  stdout.writeln('Uso: dart run tool/build_govbr_jks.dart [opcoes]');
  stdout.writeln('  --input <dir>      Pasta de origem (padrao: $_defaultInputDir)');
  stdout.writeln('  --output <file>    Arquivo JKS de saida (padrao: $_defaultOutputFile)');
  stdout.writeln('  --password <pass>  Senha do JKS (padrao: $_defaultPassword)');
}
