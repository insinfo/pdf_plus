...
import 'package:pdf_plus/src/pdf/font/ttf_writer.dart';
...
void main() {
  test('Pdf TrueType', () async {
    ...
    printTextTtf(
        ...
        g,
        s,
        File('test/assets/fonts/open-sans/OpenSans-Regular.ttf'),
        30.0 + 30.0 * top++);
    printTextTtf(
        page,
        g,
        s,
        File('test/assets/fonts/open-sans/OpenSans-Bold.ttf'),
        30.0 + 30.0 * top++);
    printTextTtf(
        page,
        g,
        s,
        File('test/assets/fonts/roboto/Roboto-Regular.ttf'),
        30.0 + 30.0 * top++);
    printTextTtf(
        page,
        g,
        s,
        File('test/assets/fonts/noto-sans/NotoSans-Regular.ttf'),
        30.0 + 30.0 * top++);
    printTextTtf(page, g, '你好 檯號 ',
        File('test/assets/fonts/GenWanMinTW-Regular.ttf'), 30.0 + 30.0 * top++);

    final file = outputFile('ttf.pdf');
    await file.writeAsBytes(await pdf.save());
  });

  test('Font SubSetting', () {
    final fontData = File('test/assets/fonts/open-sans/OpenSans-Regular.ttf')
        .readAsBytesSync();
    ...
    final output = outputFile('${font.fontName}.ttf');
    output.writeAsBytesSync(data);
  });

  test('Font SubSetting CN', () {
    final fontData =
        File('test/assets/fonts/GenWanMinTW-Regular.ttf').readAsBytesSync();
    final font = TtfParser(fontData.buffer.asByteData());
    final ttfWriter = TtfWriter(font);
